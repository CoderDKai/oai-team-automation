...
from src.email.service import unified_get_verification_code
from src.auth.crs.client import (
    crs_generate_auth_url,
    crs_exchange_code,
    crs_add_account,
    extract_code_from_url,
)
from src.auth.cpa.client import (
            ...
            if PROXY_ENABLED:
                ...
                if proxy:
                    ...
                    if proxy_type.startswith("socks"):
                        log.warning(
                            f"DrissionPage 不支持 {proxy_type} 代理，跳过代理设置"
                        )
                    else:
                        ...
                        if proxy_url:
                            ...
                            log.info(
                                f"使用代理: {proxy.get('host')}:{proxy.get('port')}"
                            )
            else:
                # 无代理时忽略系统代理
...
def check_and_handle_error_page(page, max_retries: int = 2) -> bool:
    ...
    for attempt in range(max_retries):
        # 检测错误页面
        error_text = (
            page.ele("text:糟糕，出错了", timeout=1)
            or page.ele("text:Something went wrong", timeout=1)
            or page.ele("text:Operation timed out", timeout=1)
        )
        ...
        # 点击重试按钮
        retry_btn = page.ele("text:重试", timeout=2) or page.ele(
            "text:Retry", timeout=1
        )
        if retry_btn:
            retry_btn.click()
            time.sleep(3)
    ...
    # 最后再检查一次
    error_text = page.ele("text:糟糕，出错了", timeout=1) or page.ele(
        "text:Something went wrong", timeout=1
    )
...
def wait_for_element(page, selector: str, timeout: int = 10, visible: bool = True):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if element:
                if not visible or (
                    element.states.is_displayed if hasattr(element, "states") else True
                ):
                    return element
        except Exception:
            pass
...
def wait_for_url_change(
    page, old_url: str, timeout: int = 15, contains: str = None
) -> bool:
    """等待 URL 变化
    ...
    for attempt in range(max_retries):
        try:
            ...
            if has_error:
                try:
                    retry_btn = page.ele(
                        'css:button[data-dd-action-name="Try again"]', timeout=2
                    )
                    if retry_btn:
                        log.warning(
                            f"检测到错误页面，点击重试 ({attempt + 1}/{max_retries})..."
                        )
                        retry_btn.click()
    ...
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if (
                    "页面被刷新" in error_msg
                    or "page refresh" in error_msg
                    or "stale" in error_msg
                ):
                    if attempt < max_retries - 1:
                        log.warning(
                            f"页面刷新，重试操作 ({attempt + 1}/{max_retries})..."
                        )
                        time.sleep(1)
                        continue
                raise
...
def register_openai_account(page, email: str, password: str) -> bool:
    ...
    try:
        ...
        if "auth.openai.com" in current_url:
            ...
            page_ok = (
                page.ele('css:[data-testid="signup-button"]', timeout=1)
                or page.ele("text:免费注册", timeout=1)
                or page.ele("text:Sign up", timeout=1)
                or page.ele("text:登录", timeout=1)
            )  # 也可能显示登录按钮
            if not page_ok:
            ...
            for _ in range(6):
                ...
                try:
                    email_input = page.ele(
                        'css:input[type="email"], input[name="email"]', timeout=1
                    )
        ...
        if "auth.openai.com" not in current_url and "chatgpt.com" in current_url:
            ...
            # 快速检查弹窗是否正常加载（包含登录表单）
            login_form = wait_for_element(
                page, 'css:[data-testid="login-form"]', timeout=1
            )
            if not login_form:
                login_form = page.ele("text:登录或注册", timeout=1) or page.ele(
                    "text:Log in or sign up", timeout=1
                )
            ...
            if not login_form:
                ...
                close_btn = page.ele(
                    'css:button[aria-label="Close"], button[aria-label="关闭"]',
                    timeout=1,
                )
                if not close_btn:
                ...
                signup_btn = wait_for_element(
                    page, 'css:[data-testid="signup-button"]', timeout=5
                ) or wait_for_element(page, "text:免费注册", timeout=3)
                if signup_btn:
                    signup_btn.click()
                    time.sleep(2)
                    # 再次检查弹窗
                    login_form = page.ele(
                        'css:[data-testid="login-form"]', timeout=3
                    ) or page.ele("text:登录或注册", timeout=2)
                    if not login_form:
        ...
        for step in range(max_steps):
            ...
            if "auth.openai.com/log-in-or-create-account" in current_url:
                log.step("等待邮箱输入框...")
                email_input = wait_for_element(
                    page, 'css:input[type="email"]', timeout=15
                )
                if not email_input:
                    log.error("无法找到邮箱输入框")
                    return False
            ...
            if (
                "auth.openai.com/log-in/password" in current_url
                or "auth.openai.com/create-account/password" in current_url
            ):
        ...
        if "auth.openai.com/about-you" in current_url:
            ...
            if not name_input:
                name_input = wait_for_element(
                    page, 'css:input[autocomplete="name"]', timeout=3
                )
            ...
            try:
                ...
                if error_text and error_text.states.is_displayed:
                    if code_attempt < max_code_retries - 1:
                        log.warning(
                            f"验证码错误，尝试重新获取 ({code_attempt + 1}/{max_code_retries})..."
                        )
        ...
        try:
            error_text = page.ele(
                "text:The email you provided is not supported", timeout=2
            )
            if error_text and error_text.states.is_displayed:
...
def perform_codex_authorization(page, email: str, password: str) -> dict:
    ...
    try:
        ...
        if not email_input:
            ...
            if check_and_handle_error_page(page):
                email_input = wait_for_element(
                    page, 'css:input[type="email"]', timeout=5
                )
        if not email_input:
    ...
    if (
        ...
        try:
            ...
            if not password_input:
                password_input = wait_for_element(
                    page, 'css:input[name="password"]', timeout=5
                )
...
def perform_codex_authorization_with_otp(page, email: str) -> dict:
    ...
    try:
        ...
        if "/log-in/password" in current_url or "/password" in current_url:
            ...
            otp_btn = wait_for_element(
                page, 'css:button[value="passwordless_login_send_otp"]', timeout=10
            )
            if not otp_btn:
                otp_btn = wait_for_element(
                    page, "css:button._inlinePasswordlessLogin", timeout=5
                )
            if not otp_btn:
                buttons = page.eles("css:button")
                for btn in buttons:
        ...
        for attempt in ctx.attempts():
            try:
                ...
                if AUTH_PROVIDER == "cpa":
                    ...
                    return True, None if success else (
                        True,
                        None,
                    )  # 注册成功，授权可能失败
                else:
...
def perform_cpa_authorization_with_otp(page, email: str) -> bool:
    ...
    for code_attempt in range(max_code_retries):
        try:
            ...
            try:
                error_text = (
                    page.ele("text:代码不正确", timeout=1)
                    or page.ele("text:incorrect", timeout=1)
                    or page.ele("text:Invalid code", timeout=1)
                )
                ...
                if error_text and error_text.states.is_displayed:
                    if code_attempt < max_code_retries - 1:
                        ...
                        resend_btn = (
                            page.ele("text:重新发送电子邮件", timeout=3)
                            or page.ele("text:Resend email", timeout=2)
                            or page.ele("text:resend", timeout=2)
                        )
                        if resend_btn:
                            resend_btn.click()
                            log.info("已点击重新发送，等待新验证码...")
...
def login_and_get_session(
    ...
    try:
        ...
        login_btn = wait_for_element(
            page, 'css:[data-testid="login-button"]', timeout=5
        )
        if not login_btn:
...
def login_and_authorize_team_owner(
    ...
    """
    from src.core.config import format_proxy_url
            ...
            try:
                ...
                if AUTH_PROVIDER == "cpa":
                    ...
                    if (
                        ...
                        from src.auth.s2a.client import s2a_create_account_from_oauth
                    ...
                    codex_data = perform_codex_authorization(page, email, password)
                    if codex_data:
                        from src.auth.crs.client import crs_add_account
