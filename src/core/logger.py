...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if getattr(record, "is_highlight", False):
            color = self.BLUE
...
class Logger:
    ...
    def _get_icon(self, icon: str | None = None) -> str:
        """获取图标"""
        if icon:
            return self.ICONS.get(icon, icon)
        return ""

    def info(self, msg: str, icon: str | None = None, indent: int = 0):
        """信息日志"""
        prefix = "  " * indent
    ...
    def highlight(self, msg: str, icon: str | None = None, indent: int = 0):
        """高亮日志 (蓝色)"""
        prefix = "  " * indent
...
# ==================== 配置日志辅助函数 ====================
def log_config_error(source: str, error: str, details: str | None = None):
    """记录配置加载错误
